...
def merge_csv():  # TODO 暂时不用管
    ...
    with open('DATA.csv', 'a+', newline='', encoding='gb18030') as f:
        writer = csv.writer(f, dialect="excel")
        writer.writerow(
            ['职位姓名', '留言标题', '留言标签1', '留言标签2', '留言日期', '留言内容', '回复人', '回复内容', '回复日期',
             # '满意程度',
             '解决程度分', '办理态度分', '办理速度分', '是否自动好评', '评价内容', '评价日期'])
                    ...
                    if line_count != 1:
                        writer.writerow(
                            (line[0], line[1], line[2], line[3], line[4], line[5], line[6], line[7], line[8],
                             line[9], line[10], line[11], line[12], line[13], line[14], line[15]))  # TODO 有多余元素
